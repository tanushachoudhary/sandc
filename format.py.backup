...
def _alignment_to_key(enum_val):
    """Map WD_ALIGN_PARAGRAPH to a string key for JSON/applyable spec."""
    keys = {
        WD_ALIGN_PARAGRAPH.LEFT: "left",
        WD_ALIGN_PARAGRAPH.CENTER: "center",
        WD_ALIGN_PARAGRAPH.RIGHT: "right",
...
def get_paragraph_formatting(doc, paragraph, index):
    ...
    # --- APPLYABLE SPEC (for doc-generation: apply this formatting to new docx) ---
    def _to_pt(val):
        if val is None:
            return None
        return round(val.pt, 2) if hasattr(val, "pt") else None

    def _to_in(val):
        if val is None:
            return None
        return round(val.inches, 4) if hasattr(val, "inches") else None

    line_rule = "single"
    line_value = None
    if pf.line_spacing_rule != 0 and pf.line_spacing is not None:
        line_value = round(float(pf.line_spacing), 2)
        line_rule = "multiple" if (line_value and line_value < 10) else "exact"

    style_name = getattr(paragraph.style, "name", None) if paragraph.style else None
    applyable = {
        "font_name": font_name if font_name and "(Default)" not in font_name else "Times New Roman",
        "font_size_pt": _to_pt(font_size_val) if font_size_val else 11.0,
        "alignment": _alignment_to_key(pf.alignment),
        "left_indent_in": _to_in(pf.left_indent) if pf.left_indent else 0.0,
        "right_indent_in": _to_in(pf.right_indent) if pf.right_indent else 0.0,
        "space_before_pt": _to_pt(pf.space_before) if pf.space_before else 0.0,
        "space_after_pt": _to_pt(pf.space_after) if pf.space_after else 0.0,
        ...
        "paragraph_style": style_name or "Normal",
    }
    # Section-level (same for all paragraphs from this doc; backend uses from first block)
    applyable["section"] = {
        "margin_left_in": _to_in(section.left_margin) or 1.0,
        "margin_right_in": _to_in(section.right_margin) or 1.0,
        "margin_top_in": _to_in(section.top_margin) or 1.0,
        "margin_bottom_in": _to_in(section.bottom_margin) or 1.0,
        "page_width_in": _to_in(section.page_width) or 8.5,
        "page_height_in": _to_in(section.page_height) or 11.0,
    }
